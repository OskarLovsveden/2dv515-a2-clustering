...
export const pearson = async (n, blogA, blogB) => {
    ...
    for (let i = 0; i < n; i++) {
        cntA = blogA.word_count(i) //word counts for each word in A
        cntB = blogB.word_count(i) //word counts for each word in B
    ...
    den = sqrt((sumAsq - Math.pow(sumA, 2) / n) * (sumBsq - Math.pow(sumB, 2) / n))
